...
import com.example.contractiontimer.database.AppDatabase
import com.example.contractiontimer.database.Contraction
import com.example.contractiontimer.database.ContractionDao
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var contractionDao: ContractionDao
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db = Room.databaseBuilder(
            applicationContext,
            AppDatabase::class.java, "contraction-database"
        ...
        // Load contractions from database
        contractions.addAll(contractionDao.getAll())
        ...
        buttonStartStop.setOnClickListener {
            if (!isTiming) {
                ...
                val contraction = Contraction(0, contractions.size + 1, formatTime(duration), formatTime(frequency), startTimeFormatted, endTimeFormatted)
                contractionDao.insert(contraction)
                contractions.add(0, contraction)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE_DELETE && resultCode == Activity.RESULT_OK) {
            ...
            if (contractionNumber != -1) {
                ...
                if (index != -1) {
                    val contraction = contractions.removeAt(index)
